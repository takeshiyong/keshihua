...
<script>
import { cloudControlService } from '@/api/camera';
...
export default {
  ...
  methods: {
    ...
    setSize() {
      ...
      this.screenHeight = parentDom.offsetHeight;
    },
    changeLayerSize() {
        const mobsf = this.$refs.iframe.parentNode;
        this.screenWidth = mobsf.offsetWidth;
        this.screenHeight = mobsf.offsetHeight;
    ...
    changeSize(width,height){
        ...
        this.url = `${config.PLAY_URL}/play.html?channel=${this.channelId}${type}&iframe=yes&muted=yes&stretch=yes&aspect=${width}x${height};`;
    },
    // 判断iframe加载是或否完成
    judgeIframe() {
    ...
    setUrl(channel) {
      ...
      this.url = `${config.PLAY_URL}/play.html?channel=${channel}${type}&iframe=yes&muted=yes&stretch=yes&aspect=${this.screenWidth}x${this.screenHeight}&timestamp=${new Date().getTime()};`;
    },
